...
function deObjetoAarray(objeto) {
  ...
  return Object.keys(objeto).map(propiedad => [propiedad, objeto[propiedad]]);
}
...
function capToFront(string) {
  ...
  const mayusculas = string.replace(/[^A-Z]/g, '');
  const minusculas = string.replace(/[A-Z]/g, '');
...
function asAmirror(frase) {
  ...
  const array = frase.split(' ').map(fr => fr.split('').reverse().join(''));
